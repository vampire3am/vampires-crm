...
interface PayrollRecord {
  ...
  tdsTax: number;
  netSalary: number;
  status: "DRAFT" | "REVIEWED" | "APPROVED" | "FINALIZED" | "PAID" | "REVERSED" | "CANCELLED";
  paymentDate: string;
}
...
export function HrmsWorkspace() {
  ...
  const handlePayrollTransition=async(record:PayrollRecord,status:"REVIEWED"|"APPROVED"|"FINALIZED"|"PAID")=>{
  ...
  const payrollDeductions=payrollMonthRows.reduce((sum,item)=>sum+item.otherDeduction+item.tdsTax,0);
      ...
      {activeTab === "payroll" && (
        <section className="payroll-workspace">
          ...
          <div className="payroll-progress" aria-label="Payroll approval workflow">{[
            ...
            {label:"Reviewed",status:"REVIEWED",caption:payrollRun&&payrollRun.status!=="DRAFT"?"Complete":"Pending"},
            {label:"Approved",status:"APPROVED",caption:["APPROVED","FINALIZED","PAID"].includes(payrollRun?.status??"")?"Complete":"Pending"},
            {label:"Finalized",status:"FINALIZED",caption:["FINALIZED","PAID"].includes(payrollRun?.status??"")?"Complete":"Pending"},
            {label:"Paid",status:"PAID",caption:payrollRun?.status==="PAID"?"Complete":"Pending"},
          ].map((step,index)=>{const order=["DRAFT","REVIEWED","APPROVED","FINALIZED","PAID"];const currentIndex=payrollRun?order.indexOf(payrollRun.status):-1;return <div key={step.status} className={`${index<=currentIndex?"complete":""} ${index===currentIndex?"current":""}`}><span>{index<currentIndex?<Check size={13}/>:index+1}</span><div><strong>{step.label}</strong><small>{step.caption}</small></div></div>})}</div>
          ...
          <div className="payroll-kpi-grid">
            ...
            <article><span className="rose"><UserMinus size={18}/></span><div><small>Total deductions</small><strong>NPR {payrollDeductions.toLocaleString(undefined,{minimumFractionDigits:2})}</strong><em>1% TDS and approved deductions</em></div></article>
            ...
            <article><span className="amber"><Clock size={18}/></span><div><small>Workflow status</small><strong>{payrollRun&&!["PAID","REVERSED"].includes(payrollRun.status)?1:0}</strong><em>{payrollRun?.status?.replace("_"," ")??"Not generated"}</em></div></article>
          </div>
          ...
          <div className="payroll-action-bar">
            ...
            {payrollRun?.status==="DRAFT"&&canPreparePayroll&&<button type="button" className="btn-primary" onClick={()=>void handlePayrollTransition(payrollRun,"REVIEWED")}><Check size={15}/>Complete review</button>}
            {payrollRun?.status==="REVIEWED"&&canApprovePayroll&&<button type="button" className="btn-primary" onClick={()=>void handlePayrollTransition(payrollRun,"APPROVED")}><ShieldCheck size={15}/>Approve payroll</button>}
            {payrollRun?.status==="APPROVED"&&canApprovePayroll&&<button type="button" className="btn-primary" onClick={()=>void handlePayrollTransition(payrollRun,"FINALIZED")}><ShieldCheck size={15}/>Finalize payroll</button>}
            {payrollRun?.status==="FINALIZED"&&canPayPayroll&&<button type="button" className="btn-primary" onClick={()=>void handlePayrollTransition(payrollRun,"PAID")}><Wallet size={15}/>Record payment</button>}
          ...
          <div className="payroll-main-grid">
            <div className="payroll-left-column">
              <article className="payroll-card payroll-register"><header><div><h4>Payroll summary</h4><p>{formatBsMonth(payrollMonth)} BS employee register</p></div><label><Search size={14}/><input value={payrollSearch} onChange={event=>setPayrollSearch(event.target.value)} placeholder="Search employee…"/></label></header><div className="table-wrapper"><table className="crm-table"><thead><tr><th>Employee</th><th>Basic pay</th><th>Payable days</th><th>Allowances / earnings</th><th>Deductions</th><th>Net salary</th><th>Status</th></tr></thead><tbody>{filteredPayrollRows.map(pay=><tr key={pay.id} className={previewPayslip?.id===pay.id?"selected":""} onClick={()=>setActivePayslip(pay)}><td><strong>{pay.fullName}</strong><small>{pay.empCode} · {pay.role}</small></td><td>NPR {pay.basicSalary.toLocaleString()}<small>Monthly NPR {pay.monthlyBaseSalary.toLocaleString()}</small></td><td>{pay.payableDays} / {pay.scheduledWorkingDays}</td><td>NPR {(pay.allowance+pay.commission).toLocaleString()}</td><td className="deduction-value">− NPR {(pay.otherDeduction+pay.tdsTax).toLocaleString()}</td><td><strong>NPR {pay.netSalary.toLocaleString()}</strong></td><td><span className={`payroll-status status-${pay.status.toLowerCase()}`}>{pay.status.replace("_"," ")}</span></td></tr>)}{!filteredPayrollRows.length&&<tr><td colSpan={7} className="payroll-empty-state"><Wallet size={24}/><strong>{payrollMonthRows.length?"No employees match the search":"No payroll generated for this month"}</strong><span>{payrollMonthRows.length?"Clear the search to see the full register.":"Run payroll to close attendance and create controlled employee drafts."}</span></td></tr>}</tbody></table></div><footer>Showing {filteredPayrollRows.length} of {payrollMonthRows.length} employees</footer></article>
            ...
            <aside className="payroll-card payroll-preview"><header><div><h4>Payslip preview</h4><p>{previewPayslip?.fullName??"Select an employee"}</p></div>{previewPayslip&&<button type="button" onClick={()=>setActivePayslip(previewPayslip)}>View full</button>}</header>{previewPayslip?<div className="payroll-preview-sheet"><span className="payroll-preview-brand">AECS CRM</span><h5>Abroad Education Consultancy</h5><small>Payslip · {formatBsMonth(payrollMonth)} BS</small><dl><div><dt>Employee</dt><dd>{previewPayslip.fullName}</dd></div><div><dt>Employee ID</dt><dd>{previewPayslip.empCode}</dd></div><div><dt>Payable days</dt><dd>{previewPayslip.payableDays} / {previewPayslip.scheduledWorkingDays}</dd></div></dl><h6>Earnings</h6><dl><div><dt>Prorated basic pay</dt><dd>{previewPayslip.basicSalary.toLocaleString()}</dd></div><div><dt>Allowances</dt><dd>{previewPayslip.allowance.toLocaleString()}</dd></div><div><dt>Approved earnings</dt><dd>{previewPayslip.commission.toLocaleString()}</dd></div><div className="total"><dt>Gross earnings</dt><dd>{previewPayslip.grossSalary.toLocaleString()}</dd></div></dl><h6>Deductions</h6><dl><div><dt>Income tax TDS (1%)</dt><dd>{previewPayslip.tdsTax.toLocaleString()}</dd></div><div><dt>Approved deductions</dt><dd>{previewPayslip.otherDeduction.toLocaleString()}</dd></div><div className="total"><dt>Total deductions</dt><dd>{(previewPayslip.otherDeduction+previewPayslip.tdsTax).toLocaleString()}</dd></div></dl><div className="payroll-preview-net"><span>Net payable</span><strong>NPR {previewPayslip.netSalary.toLocaleString()}</strong></div></div>:<div className="payroll-preview-empty"><FileText size={30}/><strong>No payslip available</strong><span>Generate payroll for this month to preview employee payslips.</span></div>}</aside>
          </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                  ...
                  <tbody>
                    <tr style={{ borderBottom: "1px solid #F1F5F9" }}>
                      ...
                      <td style={{ padding: "8px 12px" }}>Income Tax TDS (1% of gross)</td>
                    ...
                    <tr style={{ borderBottom: "1px solid #F1F5F9" }}>
                      ...
                      <td style={{ padding: "8px 12px" }}>Approved Other Deductions</td>
                      <td style={{ padding: "8px 12px", textAlign: "right", color: "#DC2626" }}>-₨ {activePayslip.otherDeduction.toLocaleString()}</td>
                    </tr>
                    <tr style={{ borderBottom: "1px solid #F1F5F9" }}>
                      ...
                      <td style={{ padding: "8px 12px" }}>Attendance Proration</td>
                      <td style={{ padding: "8px 12px", textAlign: "right", color: "#64748B" }}>{activePayslip.payableDays} / {activePayslip.scheduledWorkingDays} payable days</td>
                    </tr>
